...
Every handler here is plain `def`, not `async def`: `serve.py` runs one
uvicorn process, and each of these blocks briefly reading a YAML file or job
record straight off disk (or, for `say` and `cancel`, considerably longer —
see their own docstrings). A synchronous handler runs in Starlette's
threadpool instead of on the single event loop this app serves every other
request from. `tests/web/async_allowlist.py` and `tests/web/test_async_routes.py`
enforce this for the whole app, not just this module.
"""
...
def run_detail(request: Request, slug: str) -> dict:
...
def run_events(request: Request, slug: str,
...
def run_jobs(request: Request, slug: str) -> list[dict]:
...
def open_gates(request: Request,
...
def set_phase(request: Request, slug: str,
...
def advance(request: Request, slug: str) -> dict:
...
def checkpoint(request: Request, slug: str, reason: str = Form(...),
...
def resume(request: Request, slug: str) -> dict:
...
def spend(request: Request, slug: str,
...
def run_charter(request: Request, slug: str) -> dict:
...
def set_charter(request: Request, slug: str,
...
def revert_charter(request: Request, slug: str,
...
def answer(request: Request, slug: str, gate_id: str,
...
def conversation(request: Request, slug: str) -> dict:
...
def set_conversation_agent(request: Request, slug: str,
                           agent: str = Form(...)) -> dict:
